...
Fugue channel を **GKE Ingress + Cloud Endpoints DNS (`.cloud.goog` sub-domain) +
Google-managed cert + Secret Manager (token + domain)** で受け皿にする。
既存 `biblio-orchestrator` GSA に対して secret scope で `roles/secretmanager.secretAccessor`
を付与する = 新 GSA を切らない (1 workload = 1 GSA 原則、`docs/operations-runbook.md` 参照)。
...
- **ホスト名の Source of Truth = Secret Manager `fugue-domain-name`**:
  ...
  K8s Ingress manifest は envsubst で `${DOMAIN}` を展開してから apply、
  verify / 外部連携も `gcloud secrets versions access --secret=fugue-domain-name`
  経由で動的取得。
...
- **terraform CLI (v1.5+ 推奨)** — install 手順は `docs/operations-runbook.md` を参照 (AlmaLinux/RHEL/Ubuntu/Debian/macOS 対応、観測 module と共通の GCP prerequisite)
- GCP 側前提 (Cloud Endpoints API 有効化 / static IP / Secret Manager 権限) は運用 doc 参照
...
# project_id + orchestrator_gsa_email は default 削除済み、明示指定必須
...
# `PROVISIONING` + `FAILED_NOT_VISIBLE` で無限 stuck する (実 deploy で判明した
# managed-cert PROVISIONING 制約、`docs/operations-runbook.md` 参照)。
while true; do
...
3. **DNS 反映 → Ingress apply → Cert Active の順序 (実 deploy で判明した managed-cert 制約)**:
   ...
   後 15-30 分、最大 60 分)。詳細は `docs/operations-runbook.md` 参照。
...
- 参照した pattern: `terraform/m4-a-observability/` (先例、versions.tf / providers.tf の書式)
- 適用先 K8s manifest: `k8s/25-ingress-fugue-channel.yaml` (envsubst で `${DOMAIN}` を展開して apply)
- runbook: `docs/operations-runbook.md` (apply / verify / rollback 手順)
