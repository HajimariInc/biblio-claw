...
Tavily Web 検索 API key を Google Secret Manager に置くための最小構成 Terraform module (life-capabilities)。
...
# 必須 var を投入 (project_id + orchestrator_gsa_email の default 削除済み、明示指定必須)
...
```bash
cd terraform/tavily-secret
# destroy 時も必須 var 明示指定必要 (default 削除済み)
export TF_VAR_project_id='<your-gcp-project>'
export TF_VAR_orchestrator_gsa_email="biblio-orchestrator@${TF_VAR_project_id}.iam.gserviceaccount.com"
