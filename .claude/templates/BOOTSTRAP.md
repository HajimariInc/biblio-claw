# 初回起動 Bootstrap (biblio-claw, ローカル起動)

あなたは biblio-claw を新しいマシンで初めて立ち上げようとしている。開発者 (fork して動かそうとしている人) が repo を clone し、Claude Code を起動して、このファイルを読ませた状態だ。
...
- **GKE prod デプロイ** — 別 BOOTSTRAP として将来切り出し
- **GCP project / Billing / IAM の自動セットアップ** — 将来 `.sh` 化想定、今回は人手 + 誘導
- **Cloud SQL Bootstrap GRANT** — GKE prod デプロイ時に必要、ローカル起動では docker compose 内の sqlite で動く
...
- `README.md` の「クイックスタート (biblio-claw, local)」セクション — ローカル起動コマンド列の正本
...
6. 必須なのは段階 0 / 1 / 2 / 4 / 5 / 6。**GitHub App (段階 4) は棚から biblio を仕入れる機能を使い始める段階で bootstrap.md を再利用する際に必須**になるため、ローカル起動段階でも先に設定だけ済ませておく。Slack (段階 3) は任意 — CLI チャット (段階 5) が代替になるため、後回しでも bootstrap goal に到達できる。
...
`.env.example` には fork 元の参考値として project ID が記載されているが、各開発者は自分の project ID を使う。
...
- 申請から承認まで体感 **数時間〜1 営業日** (fork 元の実例)
...
`.env.example` の `ANTHROPIC_VERTEX_PROJECT_ID` には fork 元の参考値が記載されているが、開発者が違う環境であれば Edit で書き換える。
...
スクリプト内部で ADC token を取得し、OneCLI secret store に AES-256-GCM で投入する。token は ~1h で失効する設計 — ローカル起動では使う前に再実行が必要な点を開発者に共有する。
...
> **任意の理由 (代替が存在する)**: bootstrap goal (= 司書が応答する確認) は **CLI チャット (段階 5)** で十分達成できる。Slack はそれを Slack workspace 経由に拡張する位置付け。ローカル起動段階で Slack 経由の patron 体験を確認したい場合は実施するが、**bootstrap 完了の必須条件ではない**。スキップする場合は `.env` の `SLACK_*` キーを placeholder のまま残し、後で必要になったら段階 3 だけ単独で実行する。
...
biblio agent が外部 GitHub repo を読む経路 (= 棚から biblio を取り出す = 仕入れ機能の核) の認可。

> **必須の理由**: ローカル起動段階の bootstrap goal (段階 5 CLI チャット) には直接使わないが、**棚から biblio を仕入れる段階で bootstrap.md を再利用する際に必須**になる (棚からの biblio 仕入れには GitHub repo 読みが必要)。ローカル起動段階で先に設定だけ済ませて secret を OneCLI に投入できる状態を確保しておけば、仕入れ着手時に bootstrap を再走させる必要がない。

この段階では「設定を完了して secret を OneCLI に投入できる」状態を目標にする (実際の仕入れ動作確認は仕入れ機能の bootstrap で行う)。
...
4. **Webhook: Active を OFF** (ローカル起動では webhook 経路を使わない)
...
- `permissions` — 以下を含むか:
  ...
  - `metadata: read` (必須、暗黙)
- `events` — ローカル起動では空 `[]` または未設定で OK (webhook OFF)
...
スクリプト内部で PEM → RS256 JWT → installation access token を発行し OneCLI secret に upsert する。token は ~60min で失効 (ローカル起動は手動再実行、GKE prod デプロイは CronJob 自動化済)。
...
- **GKE prod デプロイ**: `README.md` の「GKE 運用メモ」セクション + `scripts/teardown-phase-2.sh` / `verify-phase-2-wiring.sh` 周辺
...
- **司書の人格 / patron 知識 / 長期記憶 の取り込み**: `.claude/templates/SOUL.md` / `USER.md` / `MEMORY.md` の biblio-claw への組み込みは **棚から biblio を仕入れる段階以降の bootstrap で扱う** (SOUL は `CLAUDE.md` から `@import` で参照、USER の preferences と MEMORY 全体は Orchestrator SQLite の動的記憶へ。本 bootstrap (ローカル起動) の責務外)
