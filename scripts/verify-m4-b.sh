#!/usr/bin/env bash
# biblio-claw: M4-B Phase 3 統合検証 (CLI channel 経由)
#
# 1 patron 命令 (= `pnpm run chat "@bot 仕入れて owner/repo"`) の ADK Runner 経由完遂を
# E2E で確定的に確認する。verify-m4-a.sh の 7-section 構造 + verify-phase-2-adk-gke.sh の
...
#   3. deploy 成立 (StatefulSet readyReplicas=1 + image tag = m4b-p3*)
#   4. 1 命令完遂 (kubectl exec で Pod 内 `pnpm run chat` 発火 → stdout に期待キーワード)
#   5. 1 trace 串刺し (Cloud Trace REST で trace + span 一覧取得 →
#      invoke_agent / execute_tool acquire_biblio / chat claude-sonnet-4-6 の 3 span 種存在)
...
# 2 連続実行で両方 exit 0 (= 冪等、副作用は draft PR のみ = 毎回別 branch で無害)。
#
...
# =============================================================================
info '=== [4/7] 1 命令完遂 (CLI channel 経由 patron 命令) ==='
...
# =============================================================================
info '=== [5/7] 1 trace 串刺し (Cloud Trace API 経由) ==='
...
# =============================================================================
info '=== [7/7] regression (verify-slack-e2e-gke.sh、opt-in) ==='
...
# PASS marker
# =============================================================================
info '  all assertions passed (preflight + keyless + deploy + 1-cmd + trace + gen_ai + regression)'
echo 'M4-B PASS'
