...
- **`InMemoryRunner` module-level singleton は Pod 再起動でロスト** — Phase 3 時点は `runEphemeral` が都度 ephemeral session を作る仕様で session 永続化なし = 無害。**(Phase 4 で状況変化)** HITL 承認 (enkin/shokyaku) の pause/resume に session 保持が必要になったため、Phase 4 で `sessionService.createSession + runner.runAsync + 明示 deleteSession` に切替済。**(issue #150 でさらに変化、2026-07-10)** 通常経路の `deleteSession` を廃止して session 継続に切替、同 Slack thread 内の会話履歴が LLM に自動 load されるようになった (deterministic sessionId + `getOrCreateSession`)。Pod 再起動時は pause 中の全 ADK session だけでなく通常経路の会話履歴も消失する = 再起動直後の同 thread 発話は履歴なしで開始される (Non-goal、`VertexAiSessionService` / `DatabaseSessionService` への切替は別 issue)。`resolveAdkApproval` の失効通知経路は Phase 4 のまま維持 (詳細は §M4-B Phase 4 §Pod 再起動時の対処 参照)。session 数管理は `src/adk/session-gc.ts` の LRU (500) + TTL (24h) sweep に一本化
...
- **dispatcher 経路の切替**: Phase 3 の `runEphemeral` (= ephemeral session 使い捨て) を **`sessionService.createSession + runner.runAsync + 明示 deleteSession`** に切替 (HITL pause で session 保持が必要になったため)。通常経路は最後に `deleteSession`、pending 経路は resume 側 (approval-dispatcher.ts) が cleanup する分業。
    - **issue #150 変更点 (2026-07-10 反映)**: 通常経路の `createSession` を `resolveAdkSessionId(channelType, platformId, threadId)` + `getOrCreateSession({sessionId})` に切替、**通常経路の finally での `deleteSession` を廃止**して session を継続。同 Slack thread 内の連続発話が同 sessionId に landing し、`Runner.runAsync` が prior events を SessionService から自動 load することで LLM prompt に前 turn の会話履歴が含まれる。CLI channel (`cli/local`) は `threadId=null` のため 1 session に集約 (Phase 1 挙動)。pending 経路の resume 側 (approval-dispatcher.ts) の `deleteSession` は温存 (破壊操作の 1 命令 1 完結 pattern を保つ)。session 総数管理は `src/adk/session-gc.ts` の LRU (MAX_SESSIONS=500) + TTL (24h) sweep (周期 1h) に一本化。Pod 再起動で in-memory session は消失する Non-goal は変更なし。
