...
### C. Milestone / Phase 別記録 (完成判定 verify + 実装ログのポインタ)
...
- **M4-A/B/C/E/F/H の各 Phase 実装記録** (実測日付・PR 番号・hash・hardening Wave 等): [Milestone 別実装記録 (Vault 移送)](#milestone-別実装記録-vault-移送) から Vault `notes/operations-log/` 配下 6 ファイルに track 別集約 (M4-A observability / M4-B ADK+HITL / M4-C reporting / M4-E Fugue channel / M4-F hybrid / M4-H ask endpoint)
...
GKE 環境の初期化 / 起動 / 状態確認 / トークン差し替え / verify / image 反映は `/init-project-gcp` skill (詳細は `.claude/commands/init-project-gcp.md` = 421 行) にサブコマンドとして集約: 引数なし (状態確認 + up) / `up` / `reset` / `refresh` / `verify` / `image-sync --tag <tag>`。

**`reset` と `image-sync` は副作用大**、実行前に `--dry-run` で削除・変更予定を確認 + 実行判断を経由する運用。実行前の状態確認は本 runbook §大原則 §コンポーネント別 早見表 で。各サブコマンドの詳細用途・期待結果・主要ログ確認経路は skill 本体および内部運用ログを参照。
...
### 自動復旧 (STALE_SESSION_RE 適用後の期待挙動)
...
## Milestone 別実装記録 (Vault 移送)

biblio-claw の Milestone / Phase 別実装記録 (実測日付 / PR 番号 / hash / hardening Wave / 内部 tag を含む) は Vault `biblio-shelf/notes/operations-log/` 配下 6 ファイルに track 別集約。各 log ファイル冒頭に「runbook 側の一般化罠との対応表」があり、本 runbook §日常運用 (OneCLI MITM / pathPattern / Vertex 401 retry / Pending Pod) の一般化罠との相互参照が可能。

| Milestone / Track | 内容 | Vault log |
| :--- | :--- | :--- |
| **M4-A Observability** | Phase 1-4 = OTel foundation / GenAI semconv / Cloud Logging → BigQuery sink / verify-m4-a 統合検証 | `notes/operations-log/m4-a-observability-log.md` |
| **M4-B ADK + HITL** | Phase 2-4 = ADK orchestrator deploy + tool routing 拡張 / slack-e2e + verify-m4-b / remaining-host-actions + HITL 統合 (9 tool + 承認カード) + Vertex 401 発生時の観察手順 | `notes/operations-log/m4-b-adk-hitl-log.md` |
| **M4-C Reporting** | Phase 1-2 = 週次 K8s CronJob + BQ + Slack owner DM / SQL 完成 + Data Table Block + cache 対称化 | `notes/operations-log/m4-c-reporting-log.md` |
| **M4-E Fugue Channel** | Phase 4-6 = observability-ad-honji (Fugue channel の OTel 2 段構造 + AD の本義) / prod-deploy (GKE Ingress + Cloud Run 実結線) / verify-fugue-channel MVP 完成判定 5 軸 assertion | `notes/operations-log/m4-e-fugue-channel-log.md` |
| **M4-F Agent-container hybrid** | Phase 1-5 = revival-core (agent-container 経路の復活) / gate-and-defense (入力ゲート 4 層 + 3 分類 routing + in-secure 3 点セット) / life-capabilities (Web 検索 + Google Drive) / progress-status (Slack 進行ステート表示) / verify-m4-f 統合 E2E | `notes/operations-log/m4-f-hybrid-log.md` |
| **M4-H ask endpoint** | Phase 3-5 = Option 1 hotfix (fugue-ask endpoint 実挙動の教訓 4 件) / Phase 3.5 system-prompt-override (fugue-ask meta response 対処) / Phase 3 life-capabilities-wiring / Phase 4 rate-limit-and-observability / Phase 5 prod-deploy | `notes/operations-log/m4-h-ask-endpoint-log.md` |

各 Milestone / Phase の**完成判定 verify script** の使い方は本 runbook 内の下記セクションが正本:

- [M2 完成判定 verify (`verify-m2.sh`)](#m2-完成判定-verifyscriptsverify-m2sh)
- [M3 完成判定 verify (`verify-m3.sh`)](#m3-完成判定-verifyscriptsverify-m3sh)
- [Phase 4 GKE deploy-verify (`verify-phase-4-deploy.sh`)](#phase-4gke-deploy-verifyverifyscriptsverify-phase-4-deploysh)
- [Phase 6 Slack E2E verify (`verify-slack-e2e-gke.sh`)](#phase-6slack-e2e-verifyscriptsverify-slack-e2e-gkesh)

**gate 4 層 (M4-F Phase 2)** と **Vertex×Claude keyless 認証** は独立 SPOT doc に集約:

- [`gate-4-layer.md`](gate-4-layer.md) — gate 4 層設計 + 3 分類 routing + in-secure 3 点セット
- [`vertex-claude-keyless.md`](vertex-claude-keyless.md) — WIF + Sidecar rotator 3 系統 + OneCLI 組み合わせ
