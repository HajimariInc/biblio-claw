...
- Fugue channel (M4-E) 単独 = fugue-http は router.ts を経由せず独自経路 = gate 無関係、GATE_ENABLED の影響を受けない (l.3164 で追加検証)
...
bash scripts/init-project-gcp-image-sync.sh --tag m4f-p4-1 --confirm
...
| `progress.status.set_typing_failed` (warn、PR #145 C2) | refresh loop + updateTypingStatus 経由の triggerTyping catch。vendor `startTyping` が Slack scope 剥奪 401 / rate limit 429 で throw。既定 4s refresh + 1s poller で高頻度発火しうる = 大量ノイズの兆候はまとまった監視で判断 | 恒常発火なら Slack app scope 確認 + Cloud Logging の 429 頻度確認 |
