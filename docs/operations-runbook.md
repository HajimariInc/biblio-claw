...
GKE 側は `k8s/10-orchestrator-statefulset.yaml` で 4 container それぞれに `LOG_FORMAT=json` + 適切な `LOG_COMPONENT` を投入済。agent Pod は `src/container-runner.ts` の `buildContainerArgs()` が host orchestrator の `LOG_FORMAT` を伝搬する。
...
| `request_id` | biblio-claw 規約 | `req_<uuid>` | patron 依頼 1 件を BQ で串刺し追跡 |
...
jsonPayload.request_id="req_<uuid>"
```
