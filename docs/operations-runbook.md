...
**目的**: Phase 1 で復活した agent-container (hybrid) 経路と既存 ADK 経路の**前段に入力ゲート 4 層を挿入**し、Layer 4 (LLM evaluator = `gemini-3.1-flash-lite`) の 3 分類 (`biblio-adk` / `biblio-other` / `in-secure`) で routing を決定する。in-secure 判定時は 3 点セット (admin DM 通知 + `gate.blocked` structured log + `.jsonl` local fallback + patron 定型文返信) を発火する。**退路 = `GATE_ENABLED=false` (既定)** で従来挙動に完全バイパス、main 合流可。

### env 一覧 (Phase 2 で追加)

| env | 既定値 | 説明 |
|:---|:---|:---|
| `GATE_ENABLED` | `false` | `'1'` or `'true'` で gate 有効化。それ以外 (未設定 / `'false'` / `'0'`) は無効化 (現状 wire の従来挙動継続 = main 合流退路) |
...
3. **両 wire 確立** (先に wire、gate 有効化は次 step で)
   - `bash scripts/init-adk-agent-gke.sh` (既存 ADK wire を restore or 新規 wire、既に wire 済なら idempotent)
   - `HYBRID_USER_ID=... HYBRID_SLACK_DM_CHANNEL_ID=... GATE_ENABLED=true bash scripts/init-hybrid-agent-gke.sh` (Phase 1 で完了済でも `GATE_ENABLED=true` env 付きで再実行 = `wireSlackDm` の `allowFanout` 経路が発火し、既存 ADK wire に対して hybrid wire を追加投入)
   - Prod DB assert: `kubectl exec biblio-orchestrator-0 -c orchestrator -n biblio-claw -- pnpm exec tsx scripts/q.ts /data/v2.db "SELECT * FROM messaging_group_agents WHERE messaging_group_id=(SELECT id FROM messaging_groups WHERE platform_id='slack:D0BB51C0KUN')"` で 2 row 確認
...
**GCP (BQ sink)**:
```sql
SELECT jsonPayload.gate_classification, COUNT(*) AS cnt
FROM `<PROJECT_ID>.<DATASET_ID>.stdout` -- + stderr UNION 対称
WHERE jsonPayload.event = 'gate.classified'
  AND timestamp > TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 1 HOUR)
GROUP BY jsonPayload.gate_classification
```

**local docker (`.jsonl` fallback)**:
```bash
docker exec biblio-orchestrator cat /app/data/gate-audit.jsonl | tail -10 | jq '.'
```
...
- Source Plan: `.claude/PRPs/plans/phase-2-gate-and-defense.plan.md` (Phase 2 完了判定成立 = Task 13-14 実測完了時に `.claude/PRPs/plans/completed/m4/m4-f/` へ archive 予定)
