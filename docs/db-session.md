...
**書き込み契約**: コンテナ内 agent-runner のみ書き込む (host は書かない)。read 失敗 (`no such table` / I/O 障害) は `getContainerState()` が `null` fallback + `log.warn` で報告 (silent-failure review 対応)、呼出側は「tool 未実行」扱いで動作継続する best-effort 契約。
