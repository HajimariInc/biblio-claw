 ...
 *      `setBiblioSetting()` で central DB に upsert → `writeBackMessage` で patron に通知
 *
 ...
 *     (= `command-gate.ts:isAdmin` と同流儀、現状運用 = 単独利用 + 内部前提)
...
registerDeliveryAction('update_config', async (content, session, inDb) => {
  ...
  if (!isAllowlistedKey(key)) {
    const allowed = BIBLIO_SETTING_KEYS.join(', ');
    ...
    await writeBackMessage(
      inDb,
      `設定エラー (invalid_key): 設定可能な key は ${allowed} のみです (指定: "${key}")。`,
      'config-resp',
      'update_config',
    );
    return;
  ...
  // 3. admin check (= 該当 agent_group に admin/owner が紐づくか、user_roles 不在なら allow-all)
  ...
  // 4. apply — DB upsert + 完了通知 + 構造化 log
  try {
    ...
    log.error('update_config setBiblioSetting threw', {
      event: 'biblio.config',
      outcome: 'failure',
      key,
      value,
