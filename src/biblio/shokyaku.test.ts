 ...
 * **cleanup 失敗時に ok=true 維持 + cleanupWarning に蓄積** の設計 (silent failure 撲滅)
 * を unit レベルで固定する。
 */
...
describe('shokyaku — cleanup 失敗時 ok=true 維持 + cleanupWarning に蓄積 (silent failure 防止)', () => {
  ...
  it('deleteFugueEquippedBiblioByName が throw でも ok=true + cleanupWarning に Fugue DB delete 失敗が立つ', async () => {
    unshelveMock.mockResolvedValue(unshelveResultOk);
    deleteEquippedMock.mockReturnValue(0);
