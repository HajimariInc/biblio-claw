/**
 * `withFugueEntrySpan` の contract test。
 *
...
describe('withFugueEntrySpan', () => {
  ...
  it('fn throw 時に recordException + ERROR status を記録し err を re-throw する', async () => {
    ...
    // 「throw 経路の fugue.outcome='error' デフォルト」の効果を直接固定化する。呼出側が
    // 成功経路で outcome を set しないまま throw されたら本 attribute が唯一の diagnostic
    // signal になる (BQ sink / Cloud Trace ダッシュボード)。この 1 行が
    // 「潰した silent failure が今後 regression しない」保証。
