/**
 * Fugue HTTP server の gate 統合 test (ask endpoint 専用)。
 *
...
describe('handleAsk gate — GATE_ENABLED=false は現状経路継続', () => {
...
describe('handleAsk gate — GATE_ENABLED=true + biblio-other = 通常経路', () => {
...
describe('handleAsk gate — GATE_ENABLED=true + biblio-adk + intent 指定 = INTENT_GATE_MISMATCH', () => {
...
describe('handleAsk gate — GATE_ENABLED=true + biblio-adk + intent 未指定 = mismatch なし', () => {
...
describe('handleAsk gate — GATE_ENABLED=true + in-secure = 200 + denied', () => {
  it('status=denied + warnings=[AD_ASK_DENIED_BY_GATE] + notifyAdmin fire-and-forget + audit outcome=blocked + fugue.ask.in_secure ログ', async () => {
    ...
    // fugue.ask.in_secure ログの構造検証 (可観測性の Contract)。
...
describe('handleAsk gate — GATE_ENABLED=true + gate throw = fail-open', () => {
