 ...
 * (Fugue channel equip-hitl 経路)。
 *
 * `session_equipped_biblios` は `sessions(id)` への FK NOT NULL + `foreign_keys=ON`
 ...
 *
 * FK なし = sessions/agent_groups への依存なし (`session_equipped_biblios` との対照)。
