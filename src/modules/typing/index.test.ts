/**
 * typing module の updateTypingStatus + currentStatus forward の unit test。
 *
 * refresh loop (4s tick + heartbeat) 本体の挙動は既存 code から無変更なので、status 関連の
 * 判定 (未起動 no-op / 初回 forward / 同値 no-op / null 遷移 / 4s tick が currentStatus を
 * forward) を集中的にカバーする。
 */
...
describe('refresh loop forwards currentStatus', () => {
...
// 実機で発見: startTypingRefresh(null) + updateTypingStatus('container 起動中') の 2 発
// fire-and-forget が Slack API 到達順で「Typing...」が後勝ちする経路あり。initialStatus 引数で
// immediate tick を最初から目的の status で発火することで race 撲滅。
describe('startTypingRefresh initialStatus (status race 解消)', () => {
...
// 4 秒周期 tick が currentStatus を実発火で forward することを検証 (中核契約: Slack 2 分自動
// クリア回避のため 4 秒毎の強制再送)。fake heartbeat 経路は必要なし = grace window (15s) 内で
// 発火する。
describe('4s tick 実発火で currentStatus を強制再送', () => {
...
// triggerTyping の empty catch を log.warn 化した「見える化」を検証する。setTyping adapter
// が throw するときに構造化ログ (event: 'progress.status.set_typing_failed') が発火し、かつ
// throw が triggerTyping の外に伝播しない (best-effort 契約) ことを assert する。
describe('triggerTyping catch 見える化', () => {
  beforeEach(() => {
...
// `progress.status.transition` structured log emit を検証。updateTypingStatus の遷移点 +
// triggerTyping の成功/失敗両分岐で emit される契約が守られていることを assert (成功パス log
// 不在 + vendor 握りつぶし観測不能を補完した実装の直接テスト)。
...
// updateTypingStatus が pausedUntil を尊重するように修正した race fix を検証。post-delivery
// pause 中に status 変化が起きても即発火せず、状態のみ更新して次 pause 明け tick が forward する。
describe('updateTypingStatus pausedUntil 尊重', () => {
  ...
  it('pause 外 (通常時) は変化検知で即発火する (regression: pausedUntil fix が過剰 skip しない)', async () => {
    startTypingRefresh('sess-1', 'ag-1', 'slack', 'U1', 'T1', 'container 起動中');
    await vi.advanceTimersByTimeAsync(0);
    calls.length = 0;
