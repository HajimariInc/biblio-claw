...
| `src/biblio/` | **biblio-claw 機能本体** (M2 PRD B + M3 装備機構)。`acquire.ts` 仕入れ / `inspect.ts` 検品 (Vertex × Gemini 3 軸) / `categorize.ts` カテゴライズ (Vertex × Claude Sonnet-4.6) / `shelve.ts` 陳列 (GitHub Git Data API + 棚リポ draft PR) / `host-proxy.ts` OneCLI proxy 経由化 (combined CA bundle で MITM/tunnel 両 trust) / `vertex-client.ts` Vertex 呼び出し (Anthropic + Gemini 両経路) / `types.ts` 型定義 + `BIBLIO_CATEGORIES` + `EquippedBiblio` / `action-helpers.ts` 4 action 共通ヘルパ (writeBackMessage + BIBLIO_NAME_RE) / `{acquire,inspect,categorize,shelve}-action.ts` delivery action handler (= agent → MCP → outbound → host → inbound 経路) / `equip.ts` 装備機構の物理配置解決 (M3 Phase 2、`session_equipped_biblios` テーブルから session 単位で DB lookup、env override は test only バックドアとして残置) |
