...
| `src/adk/runner.ts` | M4-B Phase 1: `buildRunner(agent)` factory — `new InMemoryRunner({agent, appName: 'biblio_m4b'})` を返す。外部依存ゼロの in-process `SessionService` / `ArtifactService` / `MemoryService` を自動セットアップ。M4-B Phase 2 は tool routing 拡張に集中したため差し替え見送り、Phase 3 以降で `Runner` + 永続 `SessionService` への差し替えを検討 (= 同 API 保持で factory 差し替えのみ、Slack inbound → ADK Runner 経路統合と一体で確定) |
