...
| [docs/gate-4-layer.md](docs/gate-4-layer.md) | 入力ゲート 4 層設計 (M4-F Phase 2) — pattern detection / markdown escaping / XML trust boundaries (Spotlighting) / LLM evaluator (`gemini-3.1-flash-lite`) + 3 分類 routing (`biblio-adk` / `biblio-other` / `in-secure`) + in-secure 3 点セット (admin 通知 / audit log / 定型文返信) |
