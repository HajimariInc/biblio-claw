...
| ファイル | 役割 |
|------|---------|
| `src/index.ts` | エントリーポイント: DB 初期化、マイグレーション、channel adapter、配信ポーリング、sweep、シャットダウン(SIGTERM/SIGINT で `shutdownOtel()` も実行)。main() 冒頭で `registerAnthropicVertexLlm()` を呼出 (M4-B Phase 0、ADK `LLMRegistry` への `AnthropicVertexLlm` 登録、OTel init は `--import` 経路で完了済の前提) |
...
| `src/adk/dispatcher.ts` | M4-B Phase 3: channel adapter agnostic な ADK Runner event stream → `adapter.deliver` 橋渡し dispatcher。`getSharedRunner()` = module-level singleton (SDK オブジェクト構築コスト削減、lazy 初期化)、`dispatchToAdk({channelType, platformId, threadId, patronText, requestId, ...})` = router.ts:deliverToAgent の `provider === 'adk'` 分岐から呼ばれ、`runEphemeral` event stream 消費 + `isFinalResponse` 抽出 + ADK error event 検知 + 空 patronText early feedback + throw ゼロ contract で `adapter.deliver` を呼ぶ。CLI (`cli.ts`) でも Slack (`slack.ts`) でも同一 code path。`adapter.deliver` が undefined 返却時 (CLI client 未接続等) は `not_delivered` warn で silent 化を防ぐ |
...
| `src/adk/root-agent.ts` | M4-B Phase 1: `buildRootAgent()` factory — `new LlmAgent({model: 'claude-sonnet-4-6', tools: [acquireBiblioTool, inspectBiblioTool, shelveBiblioTool]})` を返す。module-scope での `new LlmAgent` は import 時に Vertex 認証解決が走る罠があるため factory function 化 (test 環境の mock 順序問題を回避)。**M4-B Phase 3 で CLI/Slack 統合版に `ROOT_AGENT_INSTRUCTION` を拡張済** (patron 呼称 + 応答フォーマット指針 + 失敗理由伝達 + tool 使用判断規範、日本語 minimal → ~35 行) |
| `src/adk/runner.ts` | M4-B Phase 1: `buildRunner(agent)` factory — `new InMemoryRunner({agent, appName: 'biblio_m4b'})` を返す。外部依存ゼロの in-process `SessionService` / `ArtifactService` / `MemoryService` を自動セットアップ。M4-B Phase 2 は tool routing 拡張に集中したため差し替え見送り。**M4-B Phase 3 でも本ファイル自体は無変更** — CLI/Slack → ADK Runner の経路統合は `dispatcher.ts` の singleton runner 経由で成立、`InMemoryRunner` + `runEphemeral` のまま。永続 `SessionService` への差替 = multi-turn 需要が実運用で確認された時に Phase 4 判断 (= 同 API 保持で factory 差し替えのみ) |
| `src/observability/` | M4-A Phase 1+2: host 側 OTel 実装。`otel.ts` (NodeSDK + OTLP HTTP + BatchSpanProcessor low-throughput チューニング) / `auth.ts` (ADC Bearer + 45min refresh) / `env-propagation.ts` (W3C Env Carriers Spec の UPPERCASE Setter/Getter) / `no-proxy.ts` (telemetry.googleapis.com を NO_PROXY に確実に追加) / `index.ts` (公開 API) / **M4-A Phase 2 追加**: `genai.ts` (GenAI semconv 定数 + `extractVertexUsage` Gemini/Anthropic 両対応、`/incubating` import 不使用で定数 hardcode) / `trace-fields.ts` (`getTraceLogFields` = active span から Cloud Logging `logging.googleapis.com/{trace,spanId,trace_sampled}` reserved field を返す、Preferred Format = trace_id alone)。`container/agent-runner/src/observability/` と対 (Bun 非互換のため BasicTracerProvider 直接組み、auth.ts / env-propagation.ts / trace-fields.ts はコメントで同期義務を明記) |
...
| `src/router.ts` | 受信ルーティング: messaging group → agent group → session → `inbound.db` → ウェイク (M4-B Phase 3: `container_configs.provider === 'adk'` の場合は session/container 経路をスキップし `src/adk/dispatcher.ts` の in-process ADK Runner に直結、dispatcher throw 時は patron に fallback 応答を送る) |
...
| `src/db/` | DB レイヤー — agent_groups、messaging_groups、sessions、container_configs、user_roles、user_dms、pending_*、マイグレーション。`biblio-settings.ts` は biblio 設定値 (`biblio_settings` table) の 4 CRUD (個別 PRD Phase 5、`getBiblioSetting` / `setBiblioSetting` / `getAllBiblioSettings` / `deleteBiblioSetting`、`container-configs.ts` パターン mirror) |
...
| `scripts/verify-m4-a.sh` | M4-A Phase 4 統合検証 (案 C 設計、7 セクション)。Section 4 (Cloud Trace) は emit-test-span の TRACE_ID 個別マッチ assert、Section 5-6 (BQ sink) は host log が Cloud Logging に届かない plan 欠陥への対応として **TRACE_ID 個別マッチ諦め + 「過去 1h に GKE 起源の biblio.* event log が >= 1 件 BQ 到達」だけ assert** に簡素化 (= sink 疎通の証跡で M4-A Phase 3 deliverable 動作確認として十分、本番副作用なし)、Section 7 は sink filter の k8s_container + namespace 縛り保持の静的 grep。`source verify-m3-helpers.sh` で info/warn/fail を共有。必須 env = `GCP_PROJECT_ID` / `BQ_DATASET_ID`、`.env` 不在は warn 継続。終了時 `M4-A PASS` 出力 + exit 0。完全 E2E (case A = orchestrator Pod 内で fixture / case B = Slack 経由 read-only action) は将来 phase で別途検討 |
| `scripts/verify-m4-b.sh` | M4-B Phase 3 統合検証 (7 section、CLI 経由 E2E)。Section 1 (preflight) / Section 2 (keyless 4 面) / Section 3 (StatefulSet ready + image tag `m4b-p3*` + ADK agent group 存在) / **Section 4 (1 命令完遂 = `kubectl exec ... pnpm run chat "@bot 仕入れて ..."` の stdout に `仕入れ|acquire|📦` キーワード)** / Section 5 (Cloud Trace REST v1 で trace + span 一覧、`execute_tool acquire_biblio` + `chat claude-*` の 2 種存在 = `invoke_agent` は ADK 1.3.0 実装で立たない前提、Section 5 span export 遅延対策で「両 span 揃うまで retry」経路) / Section 6 (`gen_ai.operation.name` / `provider.name` / `request.model` = `chat` / `gcp.vertex_ai` / `claude-*` を AnthropicVertexLlm 自前 span でのみ確認) / Section 7 (regression = verify-slack-e2e-gke.sh opt-in)。末尾 `M4-B PASS` + 2 連続冪等 exit 0 = **M4-B PRD 完了判定**。必須 env = `GCP_PROJECT_ID` / `BQ_DATASET_ID`、任意 env = `VERIFY_M4B_BIBLIO` / `VERIFY_M4B_INCLUDE_REGRESSION` |
...
| [docs/operations-runbook.md](docs/operations-runbook.md) | **biblio-claw 運用早見表** (local / GCP)。orchestrator / agent / OneCLI の起動・ログ所在・M2/M3 verify 前提セットアップ表 + OneCLI tunnel 罠の対処 + §GKE リセット手順 (部分 reset / 完全 teardown + 再構築 + Cloud SQL Bootstrap GRANT) + /init-project / /init-project-gcp サブコマンドカタログ + §M4-A Phase 1-4 OTel 運用 (Cloud Logging → BigQuery sink Terraform 管理 + verify-m4-a.sh 統合検証手順) + §M4-B Phase 2 (ADK orchestrator deploy + tool routing 拡張 = deploy 手順 / verify 手順 / Cloud Trace 観察ガイド / 既知の罠 5 件) + §M4-B Phase 3 (CLI/Slack dispatcher 統合 = deploy 手順 / ADK agent group + CLI wire / verify 手順 / CLI 経由テスト手順 / Slack 手動デモ / Cloud Trace 観察ガイド (実観測 hierarchy) / 既知の罠 5 件 + GKE 実機 verify で判明した罠 8 件) |
