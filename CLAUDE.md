...
| [docs/operations-runbook.md](docs/operations-runbook.md) | **biblio-claw 運用早見表** (local / GCP)。orchestrator / agent / OneCLI の起動・ログ所在・M2/M3 verify 前提セットアップ表 + OneCLI tunnel 罠の対処 + §GKE リセット手順 (部分 reset / 完全 teardown + 再構築 + Cloud SQL Bootstrap GRANT) + /init-project / /init-project-gcp サブコマンドカタログ + §M4-A Phase 1-4 OTel 運用 (Cloud Logging → BigQuery sink Terraform 管理 + verify-m4-a.sh 統合検証手順) + §M4-B Phase 2 (ADK orchestrator deploy + tool routing 拡張 = deploy 手順 / verify 手順 / Cloud Trace 観察ガイド / 既知の罠 5 件) + §M4-B Phase 3 (CLI/Slack dispatcher 統合 = deploy 手順 / ADK agent group + CLI wire / verify 手順 / CLI 経由テスト手順 / Slack 手動デモ / Cloud Trace 観察ガイド (実観測 hierarchy) / 既知の罠 5 件 + GKE 実機 verify で判明した罠 8 件) + §M4-B Phase 4 (9 tool + HITL 統合 = deploy 手順 / verify 手順 (9 section 拡張) / HITL flow 全体図 / Pod 再起動時の対処 / 既知の罠 5 件) |
