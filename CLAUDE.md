...
| `src/container-runner.ts` | agent group ごとにコンテナを起動 (`getContainerRuntimeProvider()` 経由で Docker または K8s Job)。セッション DB と outbox をマウント、`getSecretProvider()` 経由で OneCLI gateway のクレデンシャルを注入(`ensureAgent` / `applyContainerSecrets`) |
| `src/adapters/container/` | `ContainerRuntimeProvider` 抽象 + `DockerContainerRuntimeProvider` (local) + `K8sJobContainerRuntimeProvider` (GKE Batch v1 Job + Informer) + factory (`CONTAINER_PROVIDER` env で切替) |
