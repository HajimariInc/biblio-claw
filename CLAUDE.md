...
`data/v2.db` はセッション専有でないすべてを保持する: users、user_roles、agent_groups、messaging_groups、wiring、pending_approvals、user_dms、chat_sdk_*(Chat SDK ブリッジ用)、boots(biblio-claw 追加、PVC + SQLite 永続化アサーション用の決定的指紋)、schema_version。マイグレーションは `src/db/migrations/` 配下に置く。
...
token 有効期限は **~60min**。期限切れ時は同スクリプトを再実行すると `PATCH /v1/secrets/:id` で value 単独更新 (id 保持、200) され、agent コンテナの次回 API 呼び出しから新トークンが見える。50 分間隔の自動再実行は M1 Phase 2 で **`k8s/30-sidecar-cronjob.yaml` (GKE CronJob `biblio-sidecar`、`*/30`) として実装済**。あわせて `set_all_agents_mode_all` で全 agent を mode=all に昇格する (Vertex スクリプトと同じ挙動) — host 起動後に新規 agent group が作られた場合は再実行で all 化される。
...
### OneCLI proxy CA bundle 投入 (GKE 経路、M2 PRD A Phase 2.5)

GKE Autopilot Warden が `autogke-no-write-mode-hostpath` で agent Pod の hostPath mount を全 deny するため、OneCLI が agent コンテナに渡したい proxy CA bundle (`/tmp/onecli-{proxy,combined}-ca.pem`) を hostPath 経由で注入できない。Phase 2.5 では K8s Secret (`biblio-onecli-ca`) として手動投入し、agent Pod に `/etc/ssl/certs/onecli/` で mount する経路を採る。`K8sJobContainerRuntimeProvider.translateSpec` が OneCLI SDK の Docker 由来 env (`HTTPS_PROXY=...@host.docker.internal:10255` / `NODE_EXTRA_CA_CERTS=/tmp/onecli-gateway-ca.pem`) を K8s-native 値 (Service DNS / Secret mount path) に rewrite する。

Secret 投入手順・再投入が必要なタイミング・マウント先の詳細は [`k8s/onecli-ca-secret.md`](k8s/onecli-ca-secret.md) を参照。Secret 不在の場合、agent Pod は `MountVolume.SetUp failed` で Pending になる (= `kubectl describe pods -l job-name=...` で診断可能)。**TODO(phase-2.6)**: OneCLI sidecar 統合 (PoC-5 写経) + emptyDir 共有で本手動投入を完全自動化予定。

### GKE 側 OneCLI への secret 投入時の `.env` 上書き罠
...
```bash
kubectl exec biblio-orchestrator-0 -n biblio-claw -- node -e "
...
**TODO(phase-2.6)**: GH App Sidecar CronJob と同様に、Vertex Bearer secret も sidecar 側で自動投入する経路に切り替えて本手動手順を廃止予定。
