...
| `src/adk/root-agent.ts` | M4-B Phase 1: `buildRootAgent()` factory — `new LlmAgent({model: 'claude-sonnet-4-6', tools: [acquireBiblioTool, inspectBiblioTool, shelveBiblioTool]})` を返す。module-scope での `new LlmAgent` は import 時に Vertex 認証解決が走る罠があるため factory function 化 (test 環境の mock 順序問題を回避)。Phase 3 で Slack 経路統合時に `instruction` 拡張予定 |
| `src/adk/runner.ts` | M4-B Phase 1: `buildRunner(agent)` factory — `new InMemoryRunner({agent, appName: 'biblio_m4b'})` を返す。外部依存ゼロの in-process `SessionService` / `ArtifactService` / `MemoryService` を自動セットアップ。Phase 2 で `Runner` + 永続 `SessionService` に差し替え予定 (= 同 API 保持で factory 差し替えのみ) |
...
| `scripts/verify-phase-1-adk-local.ts` | M4-B Phase 1 verify (= ADK Runner hierarchy の scaffolding 構造 smoke、`pnpm exec tsx --import ./src/instrumentation.ts scripts/verify-phase-1-adk-local.ts` で起動)。実 Anthropic Vertex (= ADC 経由 `claude-sonnet-4-6`) + 実 `acquire()` を呼び、stdout に `TRACE_ID` / `EVENT_COUNT` / `TOOL_CALLED` / `FINAL_TEXT` を出力。**Phase 0 `AnthropicVertexLlm` が `config.tools` を読まないため `TOOL_CALLED=true` は Phase 2 拡張後に成立**、Phase 1 では LLM が text 応答のみ返す経路で hierarchy 起動と OTel span 流出を確認する scaffolding verify。必須 env: `ANTHROPIC_VERTEX_PROJECT_ID` + ADC 済。`CLOUD_ML_REGION` 未設定は `'global'` フォールバック。GOTCHA: `HTTPS_PROXY` が `aiplatform.googleapis.com` に乗ると keyless ADC が壊れる経路あり = `unset HTTPS_PROXY` or `NO_PROXY` で対処 |
