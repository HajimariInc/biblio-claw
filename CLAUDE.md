...
| `scripts/verify-m4-a.sh` | M4-A Phase 4 統合検証 (= preflight + keyless 4 面 + emit-test-span + Cloud Trace 90s poll + BQ 5min poll + summary SQL `hit_count >= 1` + ネガティブ対照 random GHOST_TRACE_ID 0 行 + sink filter 静的反証 の 7 セクション)。`source verify-m3-helpers.sh` で info/warn/fail を共有。必須 env = `GCP_PROJECT_ID` / `BQ_DATASET_ID`、`.env` 不在は warn 継続 (GKE / CI 経路想定)。冪等性あり (2 連続実行可)、副作用なし (= shelf / DB 触らない)。終了時 `M4-A PASS` 出力 + exit 0 |
