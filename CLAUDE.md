...
biblio-shelf プロジェクトの **司書実装 repo**。**NanoClaw v2 (`nanocoai/nanoclaw` @ `2492259`, 2026-05-28) を fork して base 化**。M1 (司書骨格、GKE Autopilot `biblio-prod` 稼働) + M2 (marketplace 統合 = 仕入れ→検品→カテゴライズ→陳列) + M3 (装備 + 蔵書リスト) まで完了済、現在は独立 PRD `individual-skill-shiire` (個別 skill 仕入れ機能、Phase 1-3 完了 / Phase 4 multi-category-shelve 着手予定) が進行中。
...
| `src/biblio/` | **biblio-claw 機能本体** (M2 PRD B + 独立 PRD `individual-skill-shiire` 進行中)。`acquire.ts` 仕入れ (個別 PRD Phase 2: `ACQUIRE_SKILL_THRESHOLD` 超過で clone 前 early return → patron に個別指定促進 / 個別 PRD Phase 3: `<owner>/<repo>/<skill>` 指定時は `fetchSkillSubtree` が partial clone + sparse-checkout で該当 skill dir のみ取得 = biblioName 3 要素化) / `inspect.ts` 検品 (Vertex × Gemini 3 軸) / `categorize.ts` カテゴライズ (Vertex × Claude Sonnet-4.6) / `shelve.ts` 陳列 (GitHub Git Data API + 棚リポ draft PR) / `host-proxy.ts` OneCLI proxy 経由化 (combined CA bundle で MITM/tunnel 両 trust) / `vertex-client.ts` Vertex 呼び出し (Anthropic + Gemini 両経路) / `types.ts` 型定義 + `BIBLIO_CATEGORIES` / `action-helpers.ts` 4 action 共通ヘルパ (writeBackMessage + BIBLIO_NAME_RE) / `{acquire,inspect,categorize,shelve}-action.ts` delivery action handler (= agent → MCP → outbound → host → inbound 経路) |
| `container/agent-runner/src/mcp-tools/biblio.ts` | agent コンテナから露出する MCP ツール 4 種(`acquire_biblio` / `inspect_biblio` / `categorize_biblio` / `shelve_biblio`)。各ツールは outbound.db に system action を書き、delivery poll が `src/biblio/*-action.ts` を呼ぶ |
| `scripts/biblio-{acquire,inspect,categorize,shelve}.ts` | CLI ハーネス。`RESULT=<json>` を stdout に吐き、verify スクリプト (`scripts/verify-m2*.sh`) が assert で消費する |
