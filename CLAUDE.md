...
biblio-shelf プロジェクトの **司書実装 repo**。**NanoClaw v2 (`nanocoai/nanoclaw` @ `2492259`, 2026-05-28) を fork して base 化**。M1 (司書骨格、GKE Autopilot `biblio-prod` 稼働) + M2 (marketplace 統合 = 仕入れ→検品→カテゴライズ→陳列) まで完了済、現在は M3 (装備 + 蔵書リスト) 着手前。
...
| `src/biblio/` | **biblio-claw 機能本体** (M2 PRD B + M3 装備機構)。`acquire.ts` 仕入れ / `inspect.ts` 検品 (Vertex × Gemini 3 軸) / `categorize.ts` カテゴライズ (Vertex × Claude Sonnet-4.6) / `shelve.ts` 陳列 (GitHub Git Data API + 棚リポ draft PR) / `host-proxy.ts` OneCLI proxy 経由化 (combined CA bundle で MITM/tunnel 両 trust) / `vertex-client.ts` Vertex 呼び出し (Anthropic + Gemini 両経路) / `types.ts` 型定義 + `BIBLIO_CATEGORIES` + `EquippedBiblio` / `action-helpers.ts` 4 action 共通ヘルパ (writeBackMessage + BIBLIO_NAME_RE) / `{acquire,inspect,categorize,shelve}-action.ts` delivery action handler (= agent → MCP → outbound → host → inbound 経路) / `equip.ts` 装備機構の物理配置解決 (M3 Phase 1、env-driven stub — Phase 2 で session 単位 DB lookup へ置換予定) |
