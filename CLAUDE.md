...
| `migrate-v2.sh` + `setup/migrate-v2/` | v1→v2 マイグレーション (上流 NanoClaw 由来)。スタンドアロンスクリプト: `bash migrate-v2.sh`。DB を seed、groups / sessions をコピー、channels をインストール、コンテナをビルド、サービス切替を提案、その後 `/migrate-from-v1` skill に引き継いで owner セットアップと CLAUDE.md クリーンアップを行う。上流実装であり biblio-claw は v2 pin (`2492259`) で fork = biblio-claw ユーザには v1 → v2 マイグレは無関係。 |
...
| [docs/SECURITY.md](docs/SECURITY.md) | biblio-claw のセキュリティモデル全体像 (container 分離 + OneCLI Vault + keyless 認証 + gate 4 層) |
| [docs/vertex-claude-keyless.md](docs/vertex-claude-keyless.md) | Vertex×Claude keyless 認証設計 — WIF (Workload Identity Federation) + Sidecar token rotator 3 系統 (gh / vertex / drive) + OneCLI との組み合わせで、agent-container が生 token を持たない状態を成立させる |
| [docs/gate-4-layer.md](docs/gate-4-layer.md) | 入力ゲート 4 層設計 (M4-F Phase 2) — Unicode 正規化 (NFKC + invisible strip) / pattern detection / XML trust boundaries (Spotlighting) / LLM evaluator (`gemini-3.1-flash-lite`) + 3 分類 routing (`biblio-adk` / `biblio-other` / `in-secure`) + in-secure 3 点セット (admin 通知 / audit log / 定型文返信) |
