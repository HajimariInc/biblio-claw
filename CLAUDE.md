...
| `src/adapters/` | 環境差分吸収アダプタ群(biblio-claw 追加)。`getDsnProvider()`(DB / セッション DB のパス解決)、`getSchedulerProvider()`(sweep の tick 供給)、`getSecretProvider()`(OneCLI 操作)の 3 ファクトリ。`<X>_PROVIDER` env スイッチで Phase 2 実装に切替可能 (GkeDsnProvider 実装済) |
