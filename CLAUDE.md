...
> **Phase 単位リリース型 (M4-F 検討中、2026-07-04)**: M4-F PRD Solution Approach で提案された運用形態 = 「**Phase ごとに local 実装 + GKE deploy 確認まで完了させ、Phase 完了 = main 品質 (Prod 同等) を成立させる小刻みリリース**」。過去 PRD で base ブランチが 40-60 commits ahead まで滞留した反省 (集約 PR の起票判断コスト増 + review 対象 diff の巨大化) を回避する狙い。**将来的な運用目標**として `feature/phase-<N>-<slug>` → `main` 直接 PR を志向するが、**Phase 1 は初期整備として暫定的に `base/m4-f-agent-container-hybrid` 経由 (従来の 4 階層モデル)** で運用し、実運用の摩擦点 + 集約 PR タイミングの実測を経て Phase 2 以降で main 直接方針への移行を判断する。gate 系の切替は必ず `<FEATURE>_ENABLED` フラグ付きで合流し、main 上で即無効化できる退路を持つ。
>
