...
function makeSafeReplacer(): (key: string, value: unknown) => unknown {
  ...
  return (_key, value) => {
    if (value instanceof Error) {
      return {
        error_name: value.constructor.name,
...
function emitJson(level: Level, msg: string, data?: Record<string, unknown>): void {
  ...
  stream.write(JSON.stringify(payload, makeSafeReplacer()) + '\n');
}

function emitText(level: Level, msg: string, data?: Record<string, unknown>): void {
  if (LEVELS[level] < threshold) return;
...
const emit = FORMAT === 'json' ? emitJson : emitText;
